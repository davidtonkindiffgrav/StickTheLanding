...
# "May 5, 2025" / "Sep 5-6, 2026" — month name first, with a space before the
# day (the space-separated counterpart of _COLLAPSED_DATE_RE below). This is
# the format ProScore prints the actual competition date in, so it must be
# checked before falling back to any slash-date guess.
_MONTHFIRST_RE = re.compile(
    r"\b(" + _MONTH_NAMES + r")\s+(\d{1,2})(?:\s*-\s*\d{1,2})?,?\s+(\d{4})\b",
    re.IGNORECASE,
)
...
# Lines that carry a PDF-generation timestamp rather than the competition
# date itself (ProScore's "Printed: M/D/YYYY hh:mm", scoreholder.com's
# "Generated by scoreholder.com on D Month YYYY"). These must be stripped
# before date matching — otherwise a print/export timestamp gets mistaken
# for the event date, and _SLASH_RE's day/month guess is often wrong for it
# since it isn't reliably in Australian DD/MM order.
_EXCLUDE_LINE_RE = re.compile(r"printed:|generated by", re.IGNORECASE)


def _strip_metadata_lines(text: str) -> str:
    return "\n".join(
        line for line in text.split("\n") if not _EXCLUDE_LINE_RE.search(line)
    )
...
def extract_date_from_pdf(pdf_path) -> Optional[str]:
    ...
    for m in _MONTHFIRST_RE.finditer(text):
